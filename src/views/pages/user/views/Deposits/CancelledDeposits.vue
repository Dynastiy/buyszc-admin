<template>
  <div>
    <h3 class="font-bold text-xl">Cancelled Deposits</h3>
    <!-- Transactions Table  -->
    <section class="mt-4">
      <div class="mt-4 other--tables">
        <div class="table-responsive">
          <table class="table table-centered table-nowrap mb-0" v-if="!loading">
            ...
            <tbody>
              <tr v-for="(deposit, index) in cancelledDeposits" :key="index">
                <td>{{ index + 1 }}</td>
...
export default {
  ...
  computed: {
    cancelledDeposits() {
      return this.deposits.filter((data) => data.status == "cancelled");
    },
  },
  methods: {
